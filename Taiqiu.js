...
Vector.prototype.rotate = function() {
    var a, b;
    b = arguments;
    b.length == 1 ? (a = Math.cos(b[0]), b = Math.sin(b[0])) : (a = b[0], b = b[1]);
...
(function() {
    ...
    function d() {
        ...
        // 创建canvas包装器用于缩放
        var canvasWrapper = Quark.createDOM("div", {
            id: "canvasWrapper",
            style: {
                position: "relative",
        ...
        canvasContext = new Quark.CanvasContext({
            canvas: canvas
        });
        ...
        // 存储canvas引用供触摸控制使用
        l.canvas = canvas;
        c.canvasWrapper = canvasWrapper;
        c.canvas = canvas;
        
        // 应用移动端缩放 - 由 index.html 处理
        // 移除了对 applyWrapperScaling() 的调用以避免错误
    ...
    // 自适应屏幕尺寸 - 使用固定960x600，通过CSS缩放显示
    var screenWidth = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
    var screenHeight = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
    
    // 检测是否为移动设备
    var isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Mobile|Tablet/i.test(navigator.userAgent) || 
    ...
    window.onload = function(a) {
        ...
        id: "ballRoad",
        src: "res/ballRoad.png"
    }, {
        id: "player-txt",
        src: "res/player-txt.png"
    }, {
        id: "ball1",
        src: "res/ball1.png"
    }, {
    ...
    c.shoot = function() {
        ...
        if (c.isDown && (c.power += c.powerV, c.power > 27 || c.power < 1)) c.powerV *= -1
    };
    ...
    window.addEventListener('resize', function() {
        // 触发移动端缩放更新
        if (window.gameDeviceInfo && window.gameDeviceInfo.isMobile) {
            var event = new Event('mobileResize');
...
(function() {
    ...
    (function() {
        var g, e, c;
        g = 0;
        for (e = d.length; g < e; g++) c = d[g], c = new a(new Vector(c[0] + BallGame.offset.x, c[1] + BallGame.offset.y), new Vector(c[2] + +BallGame.offset.x, c[3] + +BallGame.offset.y)), b[g] = c
    })()
})();
(function() {
...
(function() {
    function a(a, e) {
        ...
        if (e) {
            f.fillStyle = "#eee";
            if (e > 8) f.fillRect(0, 0, 4 * c, c / 3), f.fillRect(0, 2 * c - c / 3, 4 * c, c / 3 * 2), f.fillRect(0, 4 * c - c / 3, 4 * c, c / 3), f.arc(c + 2 * c, c + 2 * c, c / 3 * 2, 0, Math.PI * 2), f.fill(), f.fillStyle = a, f.textAlign = "center", f.textBaseline = "middle", f.font = c + "px/1 Consolas, tahoma, Srial, helvetica, sans-serif", f.fillText(e, 3 * c, 3 * c);
            f.fillStyle = "#eee";
            f.beginPath();
            f.arc(c,
                c, c / 3 * 2, 0, Math.PI * 2);
    ...
    function b() {
        for (var a = !0, c = 0, e = i.length; c < e; c++) i[c].v.getLength() > 0.01 && (a = !1);
        return a
    }
    ...
    function g(a, e) {
        if (!a.isDie && !e.isDie) {
            var b = a.v.getLength(),
                f = e.v.getLength();
            if (!(b < 0.1 && f < 0.1) && (f = a.loc.minusNew(e.loc), f.getLength() <= 2 * c)) {
                var d = f.getAngle(),
                    b = Math.sin(d),
                    d = Math.cos(d);
                f.rotate(d, -b);
                ...
                a.v.x == 0 ? (e.loc.x -= e.v.x > 0 ? f : -f, e.loc.y -= e.v.y * f / Math.abs(e.v.x)) : e.v.x == 0 ? (a.loc.x -= a.v.x > 0 ? f : -f, a.loc.y -= a.v.y * f / Math.abs(a.v.x)) : a.loc.x < e.loc.x ? (a.loc.x -= f * 0.5, e.loc.x += f * 0.5) : (a.loc.x += f * 0.5, e.loc.x -= f * 0.5);
                a.loc.rotate(d, b);
                a.v.rotate(d, b);
                e.loc.rotate(d,
                    b);
    ...
    h.createBalls = function() {
        for (var b = ["#ffffff", "#E1AE07",
                "#064771", "#D7141A", "#1E1D63", "#E9520B", "#0A5326", "#900910", "#000"
            ], d = 0; d < 9; d++) k[d] = new Image, k[d].src = a(b[d], d);
        for (d = 0; d < 8; d++) k[d + 9] = new Image, k[d + 9].src = a(b[d + 1], d + 9);
        b = Math.sqrt(3) + 0.1;
        d = c;
        b = [0, -330, 0, 0, -d, b * d, d, b * d, -2 * d, 2 * b * d, 0, 2 * b * d, 2 * d, 2 * b * d, -3 * d, 3 * b * d, -d, 3 * b * d, d, 3 * b * d, 3 * d, 3 * b * d, -4 * d, 4 * b * d, -2 * d, 4 * b * d, 0, 4 * b * d, 2 * d, 4 * b * d, 4 * d, 4 * b * d];
        for (d = 0; d < 16; d++) {
            var g = new h({
                num: d,
                regX: 16,
    ...
    h.prototype.move = function() {
        this.v.getLength() < 0.1 ? (this.v.x = 0, this.v.y = 0) : (this.loc.plus(this.v), this.bounce(), this.v.scale(h.scale), this.x = this.loc.x, this.y = this.loc.y, this.bitmap.setRect([0, 0, 2 * c, 2 * c]), this.bitmap.rotation = this.v.getAngle() * 180 / Math.PI, this.checkHole())
    };
    ...
    h.prototype.inHole = function() {
        if (this.alpha < 0.1 || this.scaleX < 0.1 || this.scaleY < 0.1) {
            this.update = this.num == 0 ? null : function() {
                this.x += 4;
                this.bitmap.setRect([0, 0, 2 * c, 2 * c]);
                this.rx -= 4;
                this.bitmap.rotation = 0;
                if (this.x > e.xx) this.update = null, this.x = e.xx, e.xx = this.x - 2 * c
            };
            this.v = new Vector;
    ...
    h.prototype.checkBounds = function() {
        ...
        this.x = this.loc.x;
        this.y = this.loc.y;
        for (var a = 1, b = i.length, d = c * c * 4; a <
            b; a++) {
        ...
        this.alpha = 1;
        if (e.whiteBall.isDown && e.isDown && (this.update = this.move, this.isDie = !1, e.startShoot(), !Q.supportTouch)) e.isDown = !1
    };
...
(function() {
    ...
    b.prototype.setValue = function(a) {
        this.num = a;
        for (var a = a.toString(), b = this.children.length; a.length < b;) a = "0" + a;
        for (b -= 1; b >= 0; b--) this.getChildAt(b).setRect(this.rects[Number(a.charAt(b))])
    }
})();
(function() {
    function a(a, g) {
...
(function() {
    ...
    a.initPlayerType = function(b) {
        ...
        var c = a.player;
        b == 1 ? (a.bitmaps.ball1.x = a.playerPos[c.id - 1], a.bitmaps.ball9.x = a.playerPos[c.next.id - 1], a.player.type = b, a.player.ball = a.bitmaps.ball1, a.player.next.ball = a.bitmaps.ball9, a.player.next.ball.alpha = 0.3, a.player.next.type = -1) : (a.bitmaps.ball9.x = a.playerPos[c.id - 1], a.bitmaps.ball1.x = a.playerPos[c.next.id - 1], a.player.ball = a.bitmaps.ball9, a.player.next.ball = a.bitmaps.ball1, a.player.next.ball.alpha =
            0.3, a.player.type = b, a.player.next.type = 1)
    };
    a.initScore = function(b) {
        a.Player.score = b
    };
    a.changePlayer = function() {
        a.player == a.player1 ? (a.boardPlayer1.alpha = 0.2, a.score1.alpha = 0.2, a.boardPlayer2.alpha = 1, a.score2.alpha = 1, a.player = a.player2) : (a.boardPlayer2.alpha = 0.2, a.score2.alpha = 0.2, a.boardPlayer1.alpha = 1, a.score1.alpha = 1, a.player = a.player1);
    ...
    a.setLose = function() {
        Q.Tween.to(a.loseTxt, {
            ...
            onComplete: function() {
                ...
                a.point.visible = !1;
                a.loop = function() {};
                for (var b = 0, c = a.Ball.balls.length; b < c; b++) setTimeout(function(b) {
