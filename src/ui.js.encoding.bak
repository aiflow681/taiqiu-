(function() {
    ...
    ui.init = function() {
        ...
        var x = ns.offset.x;
        var y = ns.offset.y;
        var boardPlayer1 = ns.boardPlayer1 = new Quark.Bitmap({
            image: images["player-txt"],
            rect: [0, 0, 175, 28],
            eventEnabled: false
        });
        var boardPlayer2 = ns.boardPlayer2 = new Quark.Bitmap({
            image: images["player-txt"],
            rect: [175, 0, 175, 28],
            eventEnabled: false
        });
    ...
    ns.initScore = function() {
        ns.score1 = new ns.Num({
            max: 2,
            x: ns.offset.x + 40,
            y: ns.offset.y - 30
        });
        ns.score2 = new ns.Num({
            max: 2,
            x: ns.offset.x + 730,
            y: ns.offset.y - 30
        });
    ...
    ns.initLose = function() {
        ns.loseTxt = new Q.Bitmap({
            image: ns.images["lose"],
            x: 0,
            alpha: 0,
            y: ns.offset.y + 150,
            eventEnabled: false
            ...
            onComplete: function() {
                ...
                for (var i = 0, len = ns.Ball.balls.length; i < len; i++) {
                    setTimeout(function(i) {
                        ns.Ball.balls[i].update = ns.Ball.prototype.inHole
                    }, i * 2000, i);
                }
            }
        });
